...
class NpmCollector(BaseCollector):
    def __init__(self, client: NpmClient, packages: tuple[str, ...], max_items: int = 10) -> None:
    ...
    async def collect(self):
        ...
        items = tuple(SourceItem(source="npm", external_id=f"{r['name']}@{r['version']}", collected_at=datetime.now(UTC), payload={"title": f"{r['name']} {r['version']}", "url": r["url"], "summary": r["summary"], "published_at": r["published_at"], "category": "Developer Tools"}, metadata={"published_at": r["published_at"], "version": r["version"]}) for r in rows[:self._max_items])
