(() => {
  ...
  function highlights(value) { const parts = String(value || '').match(/[^.!?]+[.!?]+|[^.!?]+$/g) || []; return parts.map(part => part.trim()).filter(Boolean).slice(0, 3).map(part => { if (part.length <= 150) return part; const cut = part.slice(0, 150).lastIndexOf(' '); return (cut > 20 ? part.slice(0, cut) : part.slice(0, 150)).trim(); }); }
